...
cbuffer WaveTessellationBuffer : register(b2)
{
    int maxTessellation;
    int minTesselation;
    float maxHeight;
...
float CalculateLOD(float3 _position)
{
    float4 worldPosition = mul(float4(_position, 1.0f), worldMatrix);
    float height = getGerstnerWaveDisplacement(_position, waveData).y;
    float relativeHeight = height + maxHeight / maxHeight + maxHeight;
    float lod = lerp(minTesselation, maxTessellation, height / maxHeight);
    return max(lod, minTesselation);
}
